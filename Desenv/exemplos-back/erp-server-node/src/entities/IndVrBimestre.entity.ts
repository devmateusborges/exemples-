...
@Index('pk_ind_vr_bimestre', ['id'], { unique: true })
@Entity('ind_vr_bimestre', { schema: 'public' })
export class IndVrBimestre extends BaseEntity {
  ...
  @ManyToOne(() => GerEmpresa, (gerEmpresa) => gerEmpresa.indVrBimestres)
  @JoinColumn([{ name: 'ger_empresa_id', referencedColumnName: 'id' }])
  gerEmpresa: GerEmpresa;

  @ManyToOne(() => GerPer, (gerPer) => gerPer.indVrBimestres)
  @JoinColumn([{ name: 'ger_per_id', referencedColumnName: 'id' }])
  gerPer: GerPer;

  @ManyToOne(() => Ind, (ind) => ind.indVrBimestres)
