...
@Index('pk_ind_vr_quadrimestre', ['id'], { unique: true })
@Entity('ind_vr_quadrimestre', { schema: 'public' })
export class IndVrQuadrimestre extends BaseEntity {
  ...
  @ManyToOne(() => GerEmpresa, (gerEmpresa) => gerEmpresa.indVrQuadrimestres)
  @JoinColumn([{ name: 'ger_empresa_id', referencedColumnName: 'id' }])
  gerEmpresa: GerEmpresa;

  @ManyToOne(() => GerPer, (gerPer) => gerPer.indVrQuadrimestres)
  @JoinColumn([{ name: 'ger_per_id', referencedColumnName: 'id' }])
  gerPer: GerPer;

  @ManyToOne(() => Ind, (ind) => ind.indVrQuadrimestres)
