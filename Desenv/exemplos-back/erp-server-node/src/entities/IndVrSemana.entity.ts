...
@Index('pk_ind_vr_semana', ['id'], { unique: true })
@Entity('ind_vr_semana', { schema: 'public' })
export class IndVrSemana extends BaseEntity {
  ...
  @ManyToOne(() => GerEmpresa, (gerEmpresa) => gerEmpresa.indVrSemanas)
  @JoinColumn([{ name: 'ger_empresa_id', referencedColumnName: 'id' }])
  gerEmpresa: GerEmpresa;

  @ManyToOne(() => GerPer, (gerPer) => gerPer.indVrSemanas)
  @JoinColumn([{ name: 'ger_per_id', referencedColumnName: 'id' }])
  gerPer: GerPer;

  @ManyToOne(() => Ind, (ind) => ind.indVrSemanas)
