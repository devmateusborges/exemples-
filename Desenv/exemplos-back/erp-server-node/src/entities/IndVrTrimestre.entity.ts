...
@Index('pk_ind_vr_trimestre', ['id'], { unique: true })
@Entity('ind_vr_trimestre', { schema: 'public' })
export class IndVrTrimestre extends BaseEntity {
  ...
  @ManyToOne(() => GerEmpresa, (gerEmpresa) => gerEmpresa.indVrTrimestres)
  @JoinColumn([{ name: 'ger_empresa_id', referencedColumnName: 'id' }])
  gerEmpresa: GerEmpresa;

  @ManyToOne(() => GerPer, (gerPer) => gerPer.indVrTrimestres)
  @JoinColumn([{ name: 'ger_per_id', referencedColumnName: 'id' }])
  gerPer: GerPer;

  @ManyToOne(() => Ind, (ind) => ind.indVrTrimestres)
