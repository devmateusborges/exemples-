...
@Index('pk_ind_vr_quinzena', ['id'], { unique: true })
@Entity('ind_vr_quinzena', { schema: 'public' })
export class IndVrQuinzena extends BaseEntity {
  ...
  @ManyToOne(() => GerEmpresa, (gerEmpresa) => gerEmpresa.indVrQuinzenas)
  @JoinColumn([{ name: 'ger_empresa_id', referencedColumnName: 'id' }])
  gerEmpresa: GerEmpresa;

  @ManyToOne(() => GerPer, (gerPer) => gerPer.indVrQuinzenas)
  @JoinColumn([{ name: 'ger_per_id', referencedColumnName: 'id' }])
  gerPer: GerPer;

  @ManyToOne(() => Ind, (ind) => ind.indVrQuinzenas)
