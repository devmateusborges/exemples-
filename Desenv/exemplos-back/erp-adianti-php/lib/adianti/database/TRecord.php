...
abstract class TRecord
{
    ...
    /**
     * Returns the the name of the Unit Field for that Active Record
     * @return A String containing the Unit Field name
     */
    public function getUnitField()
    {
        ...
        //EJ Aletrações
        $class = get_class($this);
        if (defined("{$class}::UNITFIELD")) {
          return constant("{$class}::UNITFIELD");
        } else
        {
            return '';
        }
    ...
    public function getFirstID()
    {
        ...
        if ($conn = TTransaction::get())
        {
            ...
            $vUnitId = $this->getUnitField();
            if ($vUnitId == 'unit_id') {
                $vSqlInst = $sql->getInstruction().' where unit_id = '.TSession::getValue('userunitid');
            } else
            {
                $vSqlInst = $sql->getInstruction();
            }
            // register the operation in the LOG file
            TTransaction::log($vSqlInst);
            $result= $conn->Query($vSqlInst);
